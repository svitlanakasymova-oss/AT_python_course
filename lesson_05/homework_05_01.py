...
for car in car_data:
    year = car_data[car][1]
    engine_volume = car_data[car][2]
    price = car_data[car][-1]
    if year >= search_criteria[0]:
        if engine_volume >= search_criteria[1]:
            if price <= search_criteria[2]:
                criteria_met[car] = (car_data[car])
criteria_met = sorted(criteria_met.items(), key=lambda item: item[1][-1])
for i in range (5):
    print(criteria_met[i])